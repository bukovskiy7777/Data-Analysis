...
def create_time_features(df):
    """Создает временные признаки для модели XGBoost."""
    df['dayofmonth'] = df['Date'].dt.day
    df['dayofweek'] = df['Date'].dt.dayofweek
    # XGBoost и другие модели на деревьях часто хорошо работают с циклическими признаками,
    # но простые числовые представления (как ниже) обычно достаточны.
...
# Используем объективный подход 'reg:squarederror' для задачи регрессии
...
results_df[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].to_csv('E:\Саша\AIBY\Task 1 Forecast_XGBoost.csv', index=False)